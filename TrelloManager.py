...
class TrelloManager:
    ...
    def connect_trello(self):
        self.client = trello.TrelloClient(api_key=api_key,
                                          api_secret=api_secret,
                                          token=oauth_token,
                                          token_secret=oauth_token_secret)
    ...
    def generate_issue_from_card(self, card):

        # create issue (add card_id to redmine issue to be able to update card from redmine)
        ## call redmine manager to create issue
        ## update trello card with issue ID
        ## add activity log to card (link to redmine issue to be able to update issue from trello)
